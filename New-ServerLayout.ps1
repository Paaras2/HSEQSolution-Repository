...
# ---------------------------------------------------------------------------
Write-Step 'نشانه‌ی نسخه در هر پوشه'
# ---------------------------------------------------------------------------
# بدون این، هیچ راهی نیست که روی سرور بفهمید کدام نسخه کجا نشسته. وقتی یکی از دو
# پوشه به‌روز شود و دیگری نه، سامانه به شکل‌هایی خراب می‌شود که علتشان پیدا نیست -
# مثلاً کلاینتِ جدید کنار بک‌اندِ قدیمی، که همه‌ی فراخوان‌های API را ۴۰۴ می‌کند
# چون میان‌افزار اصلاح مسیر در آن نسخه هنوز نبوده. با این فایل، مقایسه‌ی دو پوشه
# یک نگاه است.
$stamp = @"
$($release.releaseId)
...
این فایل فقط نشانه‌ی نسخه است. اگر عدد بالا در پوشه‌ی کلاینت و پوشه‌ی بک‌اند یکی
نباشد، آن دو از یک انتشار نیستند و باید هر دو را از یک بسته کپی کنید.
"@
foreach ($target in @($siteOut, $apiOut)) {
    Set-Content -Path (Join-Path $target 'RELEASE.txt') -Value $stamp -Encoding UTF8
}
Write-Ok "RELEASE.txt در هر دو پوشه ($($release.releaseId))"
